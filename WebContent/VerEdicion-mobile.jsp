  ...
  <div class="dropdown-menu">
    <% 
    	...
    	for(int i = 0; i < nombreInstitutos.length; i++){ %>
    		<a class="dropdown-item" href="VerEdicion_mobile?nombreInstituto=<%=nombreInstitutos[i]%>"><%=nombreInstitutos[i]%></a>
    	<%}%>	
  </div>
 </div>
 <div class="btn-group">
  ...
  <div class="dropdown-menu">
    <%
    	...
    	for(int i = 0; i < nombreCategorias.length; i++){ %>
    		<a class="dropdown-item" href="VerEdicion_mobile?nombreCategoria=<%=nombreCategorias[i]%>"><%=nombreCategorias[i]%></a>
    	<%i++; 
    	}%>	
  </div>
 </div>
		...
		<%
			String nombreInstituto = (String) request.getAttribute("inst-mobile"); 
		  	...
		  	for(int i = 0; i < cursos.length; i++){%>
		  		<a href="listarEdiciones-mobile.jsp?nombreCurso=<%=cursos[i]%>" class="list-group-item list-group-item-action"><%=cursos[i] %></a>
		  	<%} %>
		</div>	
	</div>
 <%}
